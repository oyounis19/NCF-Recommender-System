# Recommender system using NCF

Welcome to the Neural Collaborative Filtering (NCF) project! This project is an End-to-End implementation of a NCF Paper using PyTorch. The model is trained on explicit and implicit feeadback data from the [MovieLens-1M] (https://grouplens.org/datasets/movielens/1m/) dataset 🎥. The model is evaluated using Hit Ratio and NDCG metrics.

## 📚 What's Inside

- **main.ipynb**: The main notebook that runs the NCF model.
- `utils/`:
  - **model.py**: The Neural Collaborative Filtering model.
  - **utils.py**: Utility functions for data processing.
- `data/`:
  - `cleaned/`: Preprocessed data (implicit and explicit feedback).
  - `ml-1m/`: Original data from the MovieLens-1M dataset.
- `server/`:
  - **main.py**: A simple FastAPI server to serve the model.
- `weights/`:
  - **ncf_model_explicit.pth**: Pretrained model weights for explicit feedback.
  - **ncf_model_implicit.pth**: Pretrained model weights for implicit feedback.

## 🌐 Overview

The project is divided into 4 main parts:

1. Data Preprocessing (explicit and implicit feedback)
2. Neural Collaborative Filtering Model
3. Training and Evaluation
4. Serving the Model using FastAPI
